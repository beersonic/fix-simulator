  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', maxWidth: '600px' }}>
      <div style={{ border: '1px solid #ddd', borderRadius: '8px', padding: '1rem', background: '#fafafa' }}>
        <h3>{session.senderCompID + ' - ' + session.targetCompID}</h3>
        ...
        <p><strong>SenderCompID:</strong> {session.senderCompID}</p>
        <p><strong>TargetCompID:</strong> {session.targetCompID}</p>
        <p><strong>Host:</strong> {session.host}</p>
        <p><strong>Port:</strong> {session.port}</p>
        <p><strong>HeartBtInt:</strong> {session.heartBtInt}</p>
        <p><strong>DefaultApplVerID:</strong> {session.defaultApplVerID}</p>
